...
import { createCloudConfig, createGeneration, createOrder, currentUser, deleteCloudConfig, downloadGeneration, getGeneration, listCloudConfigs, listGenerations, listInvoices, listOrders, listRefunds, listSessions, requestInvoice, requestRefund, retryGeneration, revokeSession, simulateOrderPayment, uploadCenterImage, type CloudConfig, type Generation, type Invoice, type Order, type RefundRequest, type Session, type User } from "../lib/api";
...
export function CloudPanel({ config, onLoad }: Props) {
  ...
  useEffect(() => {
    const sync = () => void currentUser().then((value) => {
      setUser(value);
      if (value) {
        setInvoiceEmail(value.authProvider ? "" : value.email);
        void refresh().catch((error) => setMessage(error.message));
      }
      else { setConfigs([]); setGenerations([]); setOrders([]); setSessions([]); setRefunds([]); setInvoices([]); }
    });
    sync();
  ...
  if (!user) return (
    <section className="panel cloud-panel"><div className="panel-heading"><h2>云端工作台</h2><small>配置、任务与私有下载</small></div>
      <p className="cloud-login-note">请先使用页面顶部的“登录 / 注册”进入账号，再使用云端配置、高清任务与订单功能。</p>
    </section>
  );

  return (
    <section className="panel cloud-panel"><div className="panel-heading"><h2>云端工作台</h2><div className="account-line"><span>{user.displayName || user.email} · {user.membershipLevel === "vip" ? "VIP" : "免费账户"}</span>{user.membershipLevel !== "vip" && <button type="button" onClick={() => void orderProduct("vip_monthly")}>开通 VIP ¥29.99</button>}</div></div>
